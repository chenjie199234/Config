...
//watch:    watch the change of the config
func New(timeout, interval time.Duration, path string, watch bool, selfgroup, selfname string) error {
	...
	if !watch {
		...
		for {
			...
			if e != nil {
				log.Error("[config.sdk] get config info error:", e)
				count++
				if count > 10 {
					return errors.New("[config.sdk] init config sdk error:" + e.Error())
				}
				continue
			}
		...
		c, e := api.NewSconfigWebClient(nil, selfgroup, selfname)
		if e != nil {
			return nil
		}
		count := 0

...
func (d *Dao) MongoGetInfo(ctx context.Context, groupname, appname string) (string, string, []string, error) {
	...
	if e := d.mongo.Database("s_"+groupname).Collection(appname).FindOne(ctx, bson.M{"_id": summary.CurId}).Decode(&infotemp); e != nil {
		return "", "", nil, e
	}
...
func (d *Dao) MongoGetConfig(ctx context.Context, groupname, appname, id string) (string, error) {
	...
	if e := d.mongo.Database("s_"+groupname).Collection(appname).FindOne(ctx, bson.M{"_id": objid}).Decode(&temp); e != nil {
		return "", e
	}
